...
public class Mesh {
    ...
    public Mesh(List<Vector2d> coordinates) {
        Attrib attrib;

        _vertexSize = 2;

        _data = new float[coordinates.size() * 2];
        for (int index = 0; index < coordinates.size(); index++) {
            _data[index * 2] = (float) coordinates.get(index).x;
            _data[index * 2 + 1] = (float) coordinates.get(index).y;
        }

        _indices = new int[0];

        attrib = new Attrib("coordinates", new float[]{0, 0}, 0);

        _ATTRIBS = new ArrayList<>();
    ...
    public Mesh addAttrib(String name, float[] stdData) {
        ...
        for (Attrib currentAttrib : _ATTRIBS) if (Objects.equals(currentAttrib.NAME, name)) throw new IllegalArgumentException("[MESH ERROR] : An attribute of the same name is already in this mesh!");
    ...
    public Vertex getVertex(int index) {
        ...
        return new Vertex(index * _vertexSize, _data, _ATTRIBS, _vertexSize);
    }
